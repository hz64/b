...
function (m, p) {
	...
	function u(b, c) {
		...
		if (!k)
			for (k = [], f = b.childNodes || b; null != (g = f[h]); h++) !c || e.nodeName(g, c) ? k.push(g) : e.merge(k, u(g, c));
	...
	function cb(b, c) {
		if (1 === c.nodeType && e.hasData(b)) {
			...
			var k = e._data(c, g),
				n = g.events;
			if (n)
				for (f in delete k.handle, k.events = {}, n)
					for (g = 0, h = n[f].length; h > g; g++) e.event.add(c, f, n[f][g]);
	...
	function hb(b, c) {
		if (c in b) return c;
		for (var f = c.charAt(0).toUpperCase() + c.slice(1), e = c, h = ib.length; h--;)
			if (c = ib[h] + f, c in b) return c;
		return e
	}
	...
	function ob(b, c, f) {
		for (var e, h = (va[c] || []).concat(va["*"]), k = 0, n = h.length; n > k; k++)
			if (e = h[k].call(f, c, b)) return e
	}

	function Yb(b, c) {
		var f, g, h, k, n;
		for (f in b)
			if (g = e.camelCase(f), h = c[g], k = b[f], e.isArray(k) && (h = k[1], k = b[f] = k[0]), f !== g && (b[g] = k, delete b[f]), n = e.cssHooks[g], n && "expand" in n)
				for (f in k = n.expand(k), delete b[g], k) f in b || (b[f] = k[f], c[f] = h);
			else c[g] = h
	}
	...
	function pb(b, c, f) {
		...
		f = l.props;
		for (Yb(f, l.opts.specialEasing); k > h; h++)
			if (c = ya[h].call(l, b, f, l.opts)) return c;
		return e.map(f, ob, l),
	...
	function qb(b) {
		return function (c, f) {
			...
			if (e.isFunction(f))
				for (; g = k[h++];) "+" === g.charAt(0) ? (g = g.slice(1) || "*", (b[g] = b[g] || []).unshift(f)) : (b[g] = b[g] || []).push(f)
		}
	}
	...
	e.extend({
		...
		each: function (b, c, f) {
			...
			if (f)
				if (n)
					for (; k > h && (e = c.apply(b[h], f), !1 !== e); h++);
				else
					for (h in b) {
						if (e = c.apply(b[h], f), !1 === e) break
					} else if (n)
						for (; k > h && (e = c.call(b[h], h, b[h]), !1 !== e); h++);
					else
						for (h in b)
							if (e = c.call(b[h], h, b[h]), !1 === e) break;
			return b
		},
		trim: Oa && !Oa.call("\ufeff\u00a0") ?
			function (b) {
		...
		inArray: function (b, c, f) {
			...
			if (c) {
				...
				e = c.length;
				for (f = f ? 0 > f ? Math.max(0, e + f) : f : 0; e > f; f++)
		...
		map: function (b, c, f) {
			var e, h = 0,
				...
				n = [];
			if (A(b))
				for (; k > h; h++) e = c(b[h], h, f),
					null != e && n.push(e);
			else
				for (h in b) e = c(b[h], h, f),
					null != e && n.push(e);
			return ub.apply([], n)
		},
		guid: 1,
	...
	var d = function (b) {
		function c(b, c, e, f) {
			...
			a: {
				...
				if (!f && 1 === h.length) {
					...
					for (n = j.needsContext.test(b) ? 0 : t.length; n-- && (m = t[n], !C.relative[g = m.type]);)
						if ((L = C.find[g]) && (f = L(m.matches[0].replace(oa, pa), la.test(t[0].type) && s(c.parentNode) || c))) {
		...
		function u(b, c, e) {
			...
			return c.first ?
				function (c, e, h) {
					for (; c = c[f];)
						if (1 === c.nodeType || g) return b(c, e, h)
				} : function (c, e, k) {
					var n, q, l = [Q, h];
					if (k)
						for (; c = c[f];) {
		...
		function z(b, f, h, k, n, q) {
			return k && !k[M] && (k = z(k)),
				...
				e(function (e, g, q, l) {
					...
					w = h ? n || (e ? b : p || k) ? [] : g : S;
					if (h && h(S, w, q, l), k)
						for (t = x(w, s), k(t, [], q, l), q = t.length; q--;)(m = t[q]) && (w[s[q]] = !(S[s[q]] = m));
					if (e) {
						if (n || b) {
							if (n) {
								t = [];
		...
		function y(b) {
			...
			for (var k = h ? 1 : 0, n = u(function (b) {
				...
				if (e = C.relative[b[k].type]) l = [u(v(l), e)];
				else {
		...
		var B, D, C, I, F, E, G, J, H, ma, T, ka, da, V, K, Da, Aa, M = "sizzle" + -new Date,
			...
			N = qa.indexOf ||
			function (b) {
				for (var c = 0, e = this.length; e > c; c++)
		...
		ma = c.setDocument = function (b) {
			...
			return e !== T && 9 === e.nodeType && e.documentElement ? (T = e, ka = e.documentElement, da = !F(e), b && b !== b.top && (b.addEventListener ? b.addEventListener("unload", function () {
					...
					return ka.appendChild(b).id = M, !e.getElementsByName || !e.getElementsByName(M).length
						...
						function (b) {
							...
							if (w) {
								if (h) {
									for (; e;) {
										for (t = c; t = t[e];)
											if (n ? t.nodeName.toLowerCase() === p : 1 === t.nodeType) return !1;
										s = e = "only" === b && !s && "nextSibling"
									}
									return !0
								}
								if (s = [k ? w.firstChild : w.lastChild], k && q)
									for (q = w[M] || (w[M] = {}), l = q[b] || [], L = l[0] === Q && l[1], m = l[0] === Q && l[2], t = L && w.childNodes[L]; t = ++L && t && t[e] || (m = L = 0) || s.pop();) {
	...
	e.fn.extend({
		find: function (b) {
			...
			if ("string" != typeof b) return this.pushStack(e(b).filter(function () {
				for (c = 0; h > c; c++)
					if (e.contains(g[c], this)) return !0
			}));
	...
	(e.fn.init = function (b, c) {
		...
		if ("string" == typeof b) {
			...
			if (f[1]) {
				if (c = c instanceof e ? c[0] : c, e.merge(this, e.parseHTML(f[1], c && c.nodeType ? c.ownerDocument || c : y, !0)), xb.test(f[1]) && e.isPlainObject(c))
					for (f in c) e.isFunction(this[f]) ? this[f](c[f]) : this.attr(f, c[f]);
				return this
			}
	...
	e.fn.extend({
		...
		closest: function (b, c) {
			for (var f, g = 0, h = this.length, k = [], n = i.test(b) || "string" != typeof b ? e(b, c || this.context) : 0; h > g; g++)
				for (f = this[g]; f && f !== c; f = f.parentNode)
					if (11 > f.nodeType && (n ? -1 < n.index(f) : 1 === f.nodeType && e.find.matchesSelector(f, b))) {
						k.push(f);
						break
					}
	...
	e.Callbacks = function (b) {
		...
		var c, f, g, h, k, n, q = [],
			...
			m = function (e) {
				...
				h = q.length;
				for (c = !0; q && h > k; k++)
					if (!1 === q[k].apply(e[0], e[1]) && b.stopOnFalse) {
						f = !1;
						break
					}
				c = !1;
	...
	e.extend({
		...
		when: function (b) {
			var c = 0,
				...
				q, l, m;
			if (1 < g)
				for (q = Array(g), l = Array(g), m = Array(g); g > c; c++) f[c] && e.isFunction(f[c].promise) ? f[c].promise().done(n(c, m, f)).fail(k.reject).progress(n(c, l, q)) : --h;
	...
	e.ready.promise = function (b) {
		if (!r)
			if (r = e.Deferred(), "complete" === y.readyState) setTimeout(e.ready);
	...
	e.extend({
		...
		hasData: function (b) {
			return b = b.nodeType ? e.cache[b[e.expando]] : b[e.expando], !!b && !C(b)
		},
		data: function (b, c, e) {
			return I(b, c, e)
		},
	...
	e.event = {
		...
		remove: function (b, c, f, g, h) {
			...
			if (x && (w = x.events)) {
				c = (c || "").match(W) || [""];
				for (m = c.length; m--;)
					if (q = Ab.exec(c[m]) || [], p = v = q[1], u = (q[2] || "").split(".").sort(), p) {
						t = e.event.special[p] || {};
						p = (g ? t.delegateType : t.bindType) || p;
						s = w[p] || [];
						q = q[2] && new RegExp("(^|\\.)" + u.join("\\.(?:.*\\.|)") + "(\\.|$)");
						for (l = k = s.length; k--;) n = s[k], !h && v !== n.origType || f && f.guid !== n.guid || q && !q.test(n.namespace) || g && g !== n.selector && ("**" !== g || !n.selector) || (s.splice(k, 1), n.selector && s.delegateCount--, t.remove && t.remove.call(b, n));
						l && !s.length && (t.teardown && !1 !== t.teardown.call(b, u, x.handle) || e.removeEvent(b, p, x.handle), delete w[p])
					} else
						for (p in w) e.event.remove(b, p + c[m], f, g, !0);
		...
		dispatch: function (b) {
			...
			if (q[0] = b, b.delegateTarget = this, !l.preDispatch || !1 !== l.preDispatch.call(this, b)) {
				...
				for (c = 0;
					(h = n[c++]) && !b.isPropagationStopped();)
					for (b.currentTarget = h.elem, k = 0;
		...
		handlers: function (b, c) {
			var f, g, h, k, n = [],
				q = c.delegateCount,
				l = b.target;
			if (q && l.nodeType && (!b.button || "click" !== b.type))
		...
		mouseHooks: {
			...
			filter: function (b, c) {
				var e, g, h, k = c.button,
					n = c.fromElement;
				return null == b.pageX && null != c.clientX && (g = b.target.ownerDocument || y, h = g.documentElement, e = g.body, b.pageX = c.clientX + (h && h.scrollLeft || e && e.scrollLeft || 0) - (h && h.clientLeft || e && e.clientLeft || 0), b.pageY = c.clientY + (h && h.scrollTop || e && e.scrollTop || 0) - (h && h.clientTop || e && e.clientTop || 0)), !b.relatedTarget && n && (b.relatedTarget = n === b.target ? c.toElement : n),
	...
	z.changeBubbles || (e.event.special.change = {
		...
		teardown: function () {
			return e.event.remove(this, "._change"), !bf.test(this.nodeName)
		}
	});
	...
	e.fn.extend({
		...
		off: function (b, c, f) {
			...
			return (!1 === c || "function" == typeof c) && (f = c, c = void 0), !1 === f && (f = s),
				this.each(function () {
	...
	e.extend({
		clone: function (b, c, f) {
			...
			if (z.html5Clone || e.isXMLDoc(b) || !Bb.test("<" + b.nodeName + ">") ? k = b.cloneNode(!0) : (Ra.innerHTML = b.outerHTML, Ra.removeChild(k = Ra.firstChild)), !(z.noCloneEvent && z.noCloneChecked || 1 !== b.nodeType && 11 !== b.nodeType || e.isXMLDoc(b)))
				for (g = u(k), q = u(b), n = 0; null != (h = q[n]); ++n)
					if (g[n]) {
						var m = g[n],
							w = void 0,
							t = void 0,
			...
			if (c)
				if (f)
					for (q = q || u(b), g = g || u(k), n = 0; null != (h = q[n]); n++) cb(h, g[n]);
				else cb(b, k);
			return g = u(k, "script"),
		...
		buildFragment: function (b, c, f, g) {
			for (var h, k, n, q, l, m, w, t = b.length, p = x(c), s = [], v = 0; t > v; v++)
				if (k = b[v], k || 0 === k)
					if ("object" === e.type(k)) e.merge(s, k.nodeType ? [k] : k);
					else if (mc.test(k)) {
				...
				if (!z.leadingWhitespace && Qa.test(k) && s.push(c.createTextNode(Qa.exec(k)[0])), !z.tbody)
					for (h = (k = "table" !== l || Eb.test(k) ? "<table>" !== w[1] || Eb.test(k) ? 0 : q : q.firstChild) && k.childNodes.length; h--;) e.nodeName(m = k.childNodes[h], "tbody") && !m.childNodes.length && k.removeChild(m);
			...
			for (v = 0; k = s[v++];)
				if ((!g || -1 === e.inArray(k, g)) && (n = e.contains(k.ownerDocument, k), q = u(p.appendChild(k), "script"), n && Ja(q), f))
					for (h = 0; k = q[h++];) Fb.test(k.type || "") && f.push(k);
			return p
		},
		cleanData: function (b, c) {
			for (var f, g, h, k, n = 0, q = e.expando, l = e.cache, m = z.deleteExpando, w = e.event.special; null != (f = b[n]); n++)
				if ((c || e.acceptData(f)) && (h = f[q], k = h && l[h])) {
					if (k.events)
						for (g in k.events) w[g] ? e.event.remove(f, g) : e.removeEvent(f, g, k.handle);
		...
		domManip: function (b, c) {
			...
			if (l && (k = e.buildFragment(b, this[0].ownerDocument, !1, this), f = k.firstChild, 1 === k.childNodes.length && (k = f), f)) {
				...
				for (g = h.length; l > n; n++) f = k,
					...
					c.call(this[n], f, n);
				if (g)
					for (k = h[h.length - 1].ownerDocument, e.map(h, bb), n = 0; g > n; n++) f = h[n],
	...
	var bk, Ea, vc = /^(?:toggle|show|hide)$/,
		...
		ya = [function (b, c, f) {
			...
			if (!e.isEmptyObject(t))
				for (g in v ? "hidden" in v && (u = v.hidden) : v = e._data(b, "fxshow", {}), k && (v.hidden = !u), u ? e(b).show() : p.done(function () {
						e(b).hide()
					}), p.done(function () {
						var c;
			...
			return "string" != typeof b && (f = c, c = b, b = void 0),
				...
				this.each(function () {
					...
					if (k) l[k] && l[k].stop && g(l[k]);
						...
						for (k in l) l[k] && l[k].stop && wc.test(k) && g(l[k]);
	...
	e.extend({
		valHooks: {
			...
			select: {
				get: function (b) {
					for (var c, f = b.options, g = b.selectedIndex, h = "select-one" === b.type || 0 > g, k = h ? null : [], n = h ? g + 1 : f.length, l = 0 > g ? n : h ? g : 0; n > l; l++)
						if (c = f[l], !(!c.selected && l !== g || (z.optDisabled ? c.disabled : null !== c.getAttribute("disabled")) || c.parentNode.disabled && e.nodeName(c.parentNode, "optgroup"))) {
				...
				set: function (b, c) {
					for (var f, g, h = b.options, k = e.makeArray(c), n = h.length; n--;)
						if (g = h[n], 0 <= e.inArray(e.valHooks.option.get(g), k)) try {
							g.selected = f = !0
						} catch (l) {
							g.scrollHeight
						} else g.selected = !1;
	...
	e.fn.extend({
		addClass: function (b) {
			...
			if (c)
				for (c = (b || "").match(W) || []; m > l; l++)
					if (f = this[l], g = 1 === f.nodeType && (f.className ? (" " + f.className + " ").replace(bo, " ") : " ")) {
		...
		removeClass: function (b) {
			...
			if (c)
				for (c = (b || "").match(W) || []; m > l; l++)
					if (f = this[l], g = 1 === f.nodeType && (f.className ? (" " + f.className + " ").replace(bo, " ") : "")) {
						for (k = 0; h = c[k++];)
							for (; 0 <= g.indexOf(" " + h + " ");) g = g.replace(" " + h + " ", " ");
						g = b ? e.trim(g) : "";
						f.className !== g && (f.className = g)
					}
			return this
		...
		toggleClass: function (b, c) {
			...
			return "boolean" == typeof c && "string" === f ? c ? this.addClass(b) : this.removeClass(b) : this.each(e.isFunction(b) ?
				function (f) {
					...
					if ("string" === f)
						for (var c, h = 0, k = e(this), l = b.match(W) || []; c = l[h++];) k.hasClass(c) ? k.removeClass(c) : k.addClass(c);
		...
		hasClass: function (b) {
			b = " " + b + " ";
			for (var c = 0, e = this.length; e > c; c++)
				if (1 === this[c].nodeType && 0 <= (" " + this[c].className + " ").replace(bo, " ").indexOf(b)) return !0;
			return !1
		}
	});
			...
			function f(b, c, f, g) {
				...
				if (2 !== B) {
					...
					a: {
						...
						F = f.dataTypes.slice();
						if (F[1])
							for (P in f.converters) y[P.toLowerCase()] = f.converters[P];
						for (E = F.shift(); E;)
							if (f.responseFields[E] && (H[f.responseFields[E]] = J), !N && G && f.dataFilter && (J = f.dataFilter(J, f.dataType)), N = E, E = F.shift())
								if ("*" === E) E = N;
								else if ("*" !== N && N !== E) {
							if (P = y[N + " " + E] || y["* " + E], !P)
								for (Q in y)
									if (O = Q.split(" "), O[1] === E && (P = y[N + " " + O[0]] || y["* " + O[0]])) {
			...
			var g, h, k, l, m, w, p, s, t = e.ajaxSetup({}, c),
				...
				D = {
					...
					getResponseHeader: function (b) {
						...
						if (2 === B) {
							if (!s)
								for (s = {}; c = Cc.exec(l);) s[c[1].toLowerCase()] = c[2];
							c = s[b.toLowerCase()]
						}
			...
			return {
				send: function (f, g) {
					...
					if (k.open(b.type, b.url, b.async, b.username, b.password), b.xhrFields)
						for (h in b.xhrFields) k[h] = b.xhrFields[h];
					...
					c = function (f, h) {
						var m, w, p;
						if (c && (h || 4 === k.readyState))
							if (delete Ga[l], c = void 0, k.onreadystatechange = e.noop, h) 4 !== k.readyState && k.abort();
							else {
								p = {};
								m = k.status;
...
function startGame() {
	...
	if ("" == $("#aname").val().replace(/^\s*/, "") || "" == $("#dname").val().replace(/^\s*/, "")) return alert("\u8bf7\u8f93\u5165\u540d\u5b57"), !1;
	a.part = "self";
	d.part = "match";
...
function fight(m, p, A) {
	...
	if (1 < A && (20 <= A ? (20 == A && 0 == gm && (addMsg("<span class='gmspan'>GM\u770b\u5f97\u4e0d\u8010\u70e6\u4e86\uff0c\u5077\u5077\u63d0\u9ad8\u4e86\u4e24\u4eba\u7684RP\u503c</span>", 0), gm = 1), B = Math.ceil(200 * Math.random())) : B = Math.ceil(1E3 * Math.random()), l >= B)) {
		...
		if (0 == B) {
			...
			for (G = 0; G < A; G++)
				if (l = Math.max(Math.ceil(w / 10 + Math.ceil(10 * Math.random()) - 5), 1), hurtBlood(p, l, rptime * G), addMsg("\u7830,<span class='" + p.part + "name'>[" + p.name + "]</span>\u53d7\u5230\u4e86<span class='ahurt'>" + l + "</span>\u70b9\u4f24\u5bb3", rptime * G), 0 >= p.hp) return !0;
			if (0 >= p.hp) return !0;
	...
	if (F >= B) return B = Math.floor(2 * Math.random()),
		0 == B ? addMsg("<span class='" + m.part + "name'>[" + m.name + "]</span>\u5411<span class='" + p.part + "name'>[" + p.name + "]</span>\u53d1\u8d77\u4e86\u653b\u51fb,<span class='" + p.part + "name'>[" + p.name + "]</span>\u5c31\u5730\u4e00\u6253\u6eda\uff0c\u8eb2\u8fc7\u4e86\u653b\u51fb", 0) : 1 == B && addMsg("<span class='" + m.part + "name'>[" + m.name + "]</span>\u53d1\u52a8\u4e86\u7edd\u62db\u5929\u5973\u6563\u82b1,<span class='" + p.part + "name'>[" + p.name + "]</span>\u541f\u8bd7\u4e00\u9996\u201c\u4e07\u82b1\u4e1b\u4e2d\u8fc7\uff0c\u7247\u53f6\u4e0d\u6cbe\u8eab\u201d,\u8eb2\u8fc7\u4e86\u653b\u51fb", 0), !0;
	F + H >= B && (G = !0);
